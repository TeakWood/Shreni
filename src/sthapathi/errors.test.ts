...
describe('handleCycleError', () => {
  it('pauses kshetra with cooldown on API_DOWN (HTTP 503)', async () => {
    ...
    expect(state.kshetras['myapp'].requiresManualResume).toBe(false);
    expect(state.kshetras['myapp'].reason).toBe('api_down');
  });
...
describe('notifyOperator', () => {
  it('writes a notification with cause + remediation to the notifications feed', async () => {
    const err = vi.spyOn(console, 'error').mockImplementation(() => {});
    await notifyOperator(KSHETRA, TASK, 'stuck', 'agent appears hung', '  1) restart it');

    const feed = join(dir, '.shreni', 'kshetra', 'myapp', 'notifications.jsonl');
    const entry = JSON.parse(readFileSync(feed, 'utf8').trim().split('\n').pop()!);
    expect(entry.event).toBe('stuck');
    expect(entry.reason).toBe('agent appears hung');
    expect(entry.message).toContain('agent appears hung');
    expect(entry.message).toContain('restart it');
    expect(err).toHaveBeenCalledWith(expect.stringContaining('agent appears hung'));
    err.mockRestore();
  });

  it('never throws even without a task or remediation', async () => {
    const err = vi.spyOn(console, 'error').mockImplementation(() => {});
    await expect(notifyOperator(KSHETRA, null, 'git_failed')).resolves.toBeUndefined();
    err.mockRestore();
  });
});
