/**
 * E2E integration test: Claude Code files task → Sthapathi picks up (the test design §7.3)
 *
...
const { selectNext, prepareTask } = await import('../sthapathi/pickup.js');

// Mirror the scheduler cycle (the Sthapathi workflow design §4.1): SELECT
// (read-only) then PREPARE (claim + work-tree setup). Returns the prepared task
// or null if nothing was ready / preflight rejected.
async function pickupCycle(k: KshetraConfig): Promise<Task | null> {
  const selected = await selectNext(k);
  if (!selected) return null;
  return prepareTask(selected, k);
}
...
describe('pickup cycle (unit)', () => {
  it('claims the task and returns it when bd ready returns a ready issue', async () => {
    ...
    const task = await pickupCycle(KSHETRA);
    expect(task).not.toBeNull();
    expect(task?.id).toBe('myapp-123');
    expect(mockClaim).toHaveBeenCalledWith('myapp-123');
  });
  ...
  it('selection is read-only: no beads sync or git until PREPARE', async () => {
    mockReady.mockResolvedValue(taskJson());
  ...
  it('PREPARE syncs beads before mutating the work tree (checkout)', async () => {
    mockReady.mockResolvedValue(taskJson());
    await pickupCycle(KSHETRA);
    expect(mockSyncBeads).toHaveBeenCalledBefore(mockCheckout as ReturnType<typeof vi.fn>);
  });
});
