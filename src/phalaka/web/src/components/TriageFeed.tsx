...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      type="button"
      onClick={copy}
      className="shrink-0 text-xs px-2 py-0.5 rounded bg-slate-700 hover:bg-slate-600 text-slate-200"
    >
      {copied ? 'Copied!' : 'Copy'}
    </button>
  );
...
function TriageRow({ entry }: { entry: TriageEntry }) {
  return (
    <div className="px-3 py-2 border-b border-slate-800">
      ...
      <div className="mt-1 text-xs text-slate-300">{entry.reason}</div>
      <pre className="mt-2 px-2 py-1.5 rounded bg-slate-950 text-xs text-emerald-200 whitespace-pre-wrap overflow-x-auto">
        {entry.remediation}
      </pre>
    </div>
  );
  ...
  return (
    <section className="mb-6 rounded border border-slate-800 overflow-hidden">
      <div className="flex items-center gap-3 px-3 py-2 bg-slate-800">
        <h2 className="text-sm font-semibold text-slate-100">Needs a human</h2>
        <span
          className={
