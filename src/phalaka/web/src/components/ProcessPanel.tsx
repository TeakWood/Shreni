...
function ProcessRow({ snap }: { snap: ProcessSnapshot }) {
  return (
    <div className="flex items-center gap-3 px-3 py-2 border-b border-slate-800 light:border-slate-200">
      ...
      <span className="flex-1 text-sm text-slate-200 light:text-slate-800">{processLabel(snap)}</span>
      {snap.activeBead ? (
        <span className="text-xs text-slate-400 light:text-slate-600 font-mono" title={snap.activeBead.title}>
          {snap.activeBead.id}
        </span>
      ) : null}
      {snap.phase ? (
        <span className="text-xs px-1.5 py-0.5 rounded bg-slate-700 text-slate-300 light:bg-slate-200 light:text-slate-700 font-mono">{snap.phase}</span>
      ) : null}
      ...
      <span className="text-xs text-slate-600 light:text-slate-400 font-mono">pid {snap.pid}</span>
    </div>
  );
}
  ...
  return (
    <section className="mb-6 rounded border border-slate-800 light:border-slate-200 overflow-hidden">
      <div className="flex items-center gap-3 px-3 py-2 bg-slate-800 light:bg-slate-100">
        <h2 className="text-sm font-semibold text-slate-100 light:text-slate-900">Processes</h2>
        <span className={'text-xs ' + (streamStatus === 'live' ? 'text-emerald-400 light:text-emerald-600' : 'text-slate-500')}>
